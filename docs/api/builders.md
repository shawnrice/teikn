...
The primary way to create tokens. Accepts values in three forms: raw, tuple (with description), or
object (with description and modes).
...
References use `{tokenName}` syntax and are resolved by `resolveReferences()` (called automatically
...
The name stands for _density-independent pixel_, borrowed from Android's display system.
