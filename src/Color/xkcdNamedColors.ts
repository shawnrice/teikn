...
export type XKCDColor = keyof typeof xkcdColors;

const xkcdColorsByValue = flip(xkcdColors);

export type XKCDColorValue = keyof typeof xkcdColorsByValue;

const colors = Object.entries(xkcdColors).map(([name, hex]) => ({ name, hex }));
// We should lazy-create this so that we don't take the memory hit if we don't need it
const index = new Octree(colors, { key: 'hex', transform: hexToRGB });

type XKCDColorSearchResult = {
  name: XKCDColor;
...
export function closest(color: Color | string): XKCDColorSearchResult | null {
  if (typeof color === 'string') {
    // Check that this is hex (it can be 6 or 3 digits)
    if (!isHexColor(color)) {
      throw new Error('Invalid color, must be a hex');
    }

    // We need the # to check -- is this true?
    if (!color.startsWith('#')) {
    ...
    if (color.length === 4) {
      console.log('redo as six');
      const [, r, g, b] = color;
      return closest(`#${r}${r}${g}${g}${b}${b}`);
    }

    console.log('its a string');

    // If it's 6 digits, we're good
    return index.closest(color);
  }

  // Search based on the hex code
  return index.closest(color.toString('hex'));
}
