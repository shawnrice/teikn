...
const help = () => {
  ...
  console.log(
    '*',
    caller.join(' '),
    'path/to/tokens.ts --outDir=path/to/out --generators="Scss,Json,JavaScript,CssVars,Html" --plugins="ColorTransformPlugin,ScssQuoteValuePlugin"',
  );
  console.log();
...
const commands = { help, list, validate: validateTokens, version: () => console.log(signature()) };
...
const usage = () => {
  ...
  console.log(
    'Usage:',
    caller.join(' '),
    'path/to/tokens.ts --outDir=path/to/out --generators="Scss,Json,JavaScript" --plugins="ColorTransformPlugin,ScssQuoteValuePlugin"',
  );
  process.exit(2);
};
