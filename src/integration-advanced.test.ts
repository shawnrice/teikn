...
const spacing = scale('spacing', { sm: dp(8), md: dp(16), lg: dp(24) });
...
const borders = composite('border', {
  thin: { width: dp(1), style: 'solid', color: new Color('#cccccc') },
});

const durations = group('duration', { fast: dur(200, 'ms'), slow: dur(500, 'ms') });
...
describe('theme with Color object overrides', () => {
  const dark = theme('dark', colors, { primary: new Color('#6db3f8') });
  ...
  test('theme overrides do not mutate original Color tokens', () => {
    ...
    const writer = new Teikn({ generators: [new Teikn.generators.Json()], themes: [dark] });
