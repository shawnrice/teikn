...
describe('builders', () => {
  describe('group', () => {
    test('creates tokens from plain values', () => {
      const result = group('color', { primary: '#0066cc', secondary: '#cc6600' });
    ...
    test('creates tokens from [value, usage] tuples', () => {
      const result = group('color', { primary: ['#0066cc', 'Main brand color'] });
    ...
    test('named properties unwrap object input values', () => {
      const result = group('color', { surface: { value: '#fff', modes: { dark: '#1a1a1a' } } });
  ...
  describe('composite', () => {
    test('creates composite tokens', () => {
      const result = composite('typography', {
        heading: { fontFamily: 'Rubik', fontSize: '2rem', fontWeight: 700, lineHeight: 1.2 },
      });
    ...
    test('creates composite with object-style input without modes', () => {
      const result = composite('border', {
        card: { value: { width: '1px', style: 'solid' }, usage: 'Card border' },
      });
  ...
  describe('theme', () => {
    test('creates a theme layer from tokens', () => {
      const colors = group('color', { background: '#ffffff', text: '#333', primary: '#0066cc' });
      ...
      expect(dark.overrides).toEqual({ 'color.background': '#1a1a1a', 'color.text': '#eee' });
    });

    test('throws on unknown token name', () => {
      const colors = group('color', { background: '#ffffff', text: '#333' });

      expect(() => theme('dark', colors, { background: '#1a1a1a', typo: '#red' } as any)).toThrow(
        'unknown token "typo"',
      );
    });
    ...
    test('derived theme can override parent values', () => {
      ...
      const darkHighContrast = theme('dark-hc', dark, { background: '#000000', text: '#ffffff' });
    ...
    test('derived theme throws on unknown token', () => {
      ...
      expect(() => theme('bad', dark, { nonexistent: '#red' } as any)).toThrow(
        'unknown token "nonexistent"',
      );
    });

    test('works with merged tokens() output across groups', () => {
      const colors = group('color', { background: '#ffffff', textPrimary: '#333' });
      const spacing = group('spacing', { sm: '0.5rem', md: '1rem' });

      const allTokens = tokens(colors, spacing);

      const dark = theme('dark', allTokens, { background: '#1a1a1a', textPrimary: '#eee' });
    ...
    test('throws on unknown token when using merged tokens() output', () => {
      ...
      expect(() => theme('dark', allTokens, { nonexistent: '#red' } as any)).toThrow(
        'unknown token "nonexistent"',
      );
    });
    ...
    test('throws on ambiguous bare token names across groups', () => {
      ...
      expect(() => theme('dark', allTokens, { primary: '#3399ff' })).toThrow(
        'ambiguous token name "primary"',
      );
    });
    ...
    test('qualified override key disambiguates across groups', () => {
      ...
      const dark = theme('dark', allTokens, { 'color.primary': '#3399ff' });
    ...
    test('two qualified overrides for distinct tokens sharing a bare name coexist', () => {
      ...
      const dark = theme('dark', allTokens, { 'color.primary': '#3399ff', 'size.primary': '20px' });
      expect(dark.overrides['color.primary']).toBe('#3399ff');
  ...
  describe('preview hint (object type argument)', () => {
    test('group stamps every token with the preview kind', () => {
      const result = group(
        ...
        { card: '0 1px 2px #0003', modal: '0 8px 16px #0003' },
      );
