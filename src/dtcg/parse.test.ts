...
describe('parseDtcg', () => {
  ...
  test('groups inherit $type to children', () => {
    const doc: DtcgDocument = {
      color: {
        ...
        secondary: { $value: { colorSpace: 'srgb', components: [0, 0, 1] } },
      },
    };
    const tokens = parseDtcg(doc);
  ...
  test('child $type overrides inherited type', () => {
    const doc: DtcgDocument = {
      tokens: { $type: 'color', size: { $value: { value: 16, unit: 'px' }, $type: 'dimension' } },
    };
    const tokens = parseDtcg(doc);
  ...
  test('custom separator works', () => {
    const doc: DtcgDocument = {
      color: { primary: { $value: { colorSpace: 'srgb', components: [1, 0, 0] }, $type: 'color' } },
    };
  ...
  test('dimension values are converted to Dimension instances', () => {
    const doc: DtcgDocument = {
      spacing: {
        ...
        md: { $value: { value: 1, unit: 'rem' }, $type: 'dimension' },
      },
    };
    const tokens = parseDtcg(doc);
    expect(tokens[0]!.value).toBeInstanceOf(Dimension);
  ...
  test('duration values are converted to Duration instances', () => {
    const doc: DtcgDocument = {
      ...
      slow: { $value: { value: 0.5, unit: 's' }, $type: 'duration' },
    };
    const tokens = parseDtcg(doc);
  ...
  test('cubicBezier values are converted to CubicBezier instances', () => {
    const doc: DtcgDocument = { ease: { $value: [0.42, 0, 0.58, 1], $type: 'cubicBezier' } };
    const tokens = parseDtcg(doc);
  ...
  test('number type is preserved', () => {
    const doc: DtcgDocument = { opacity: { $value: 0.5, $type: 'number' } };
    const tokens = parseDtcg(doc);
  ...
  test('fontFamily string is preserved', () => {
    const doc: DtcgDocument = { body: { $value: 'Arial', $type: 'fontFamily' } };
    const tokens = parseDtcg(doc);
  ...
  test('fontFamily array is joined', () => {
    const doc: DtcgDocument = {
      body: { $value: ['Roboto', 'Arial', 'sans-serif'], $type: 'fontFamily' },
    };
    const tokens = parseDtcg(doc);
  ...
  test('fontWeight is preserved', () => {
    const doc: DtcgDocument = { bold: { $value: 700, $type: 'fontWeight' } };
    const tokens = parseDtcg(doc);
  ...
  test('strokeStyle string is preserved', () => {
    const doc: DtcgDocument = { border: { $value: 'dashed', $type: 'strokeStyle' } };
    const tokens = parseDtcg(doc);
  ...
  test('strokeStyle object is preserved', () => {
    const doc: DtcgDocument = {
      border: { $value: { dashArray: [2, 4], lineCap: 'round' }, $type: 'strokeStyle' },
    };
    const tokens = parseDtcg(doc);
  ...
  test('fontStyle is preserved', () => {
    const doc: DtcgDocument = { italic: { $value: 'italic', $type: 'fontStyle' } };
    const tokens = parseDtcg(doc);
  ...
  test('parses mode values from $extensions.mode', () => {
    const doc: DtcgDocument = {
      surface: {
        ...
        $type: 'color',
        $extensions: { mode: { dark: { colorSpace: 'srgb', components: [0.1, 0.1, 0.1] } } },
      },
    };
  ...
  test('$extensions.mode value of null does not crash the parser', () => {
    const doc: DtcgDocument = {
      surface: {
        $value: { colorSpace: 'srgb', components: [1, 1, 1] },
        $type: 'color',
        $extensions: { mode: { dark: null as never } },
      },
    };
