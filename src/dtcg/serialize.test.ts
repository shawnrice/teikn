...
describe('serializeDtcg', () => {
  test('serializes a simple color token', () => {
    ...
    expect(primary.$value).toEqual({ colorSpace: 'srgb', components: [1, 0, 0] });
  ...
  test('round-trip: parse then serialize produces equivalent data', () => {
    const original = {
      color: {
        $type: 'color',
        red: { $value: { colorSpace: 'srgb', components: [1, 0, 0] }, $description: 'Pure red' },
        blue: { $value: { colorSpace: 'srgb', components: [0, 0, 1] } },
      },
      spacing: { $type: 'dimension', sm: { $value: { value: 8, unit: 'px' } } },
    };
  ...
  test('mode values go through teiknValueToDtcg conversion', () => {
    const tokens: Token[] = [
      { name: 'gap', value: '16px', type: 'spacing', modes: { compact: '8px' } },
    ];
    const doc = serializeDtcg(tokens);
