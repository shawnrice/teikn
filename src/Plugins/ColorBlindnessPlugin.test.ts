...
const nonColorToken = (name: string): Token => ({ name, value: '16px', type: 'dimension' });
...
describe('ColorBlindnessPlugin', () => {
  ...
  test('token names follow suffix pattern', () => {
    const plugin = new ColorBlindnessPlugin({ types: ['protanopia', 'deuteranopia'] });
  ...
  test('token names follow custom suffix pattern', () => {
    const plugin = new ColorBlindnessPlugin({ types: ['tritanopia'], suffix: '--{type}' });
